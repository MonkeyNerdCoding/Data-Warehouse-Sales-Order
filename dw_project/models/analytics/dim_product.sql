...
SELECT
    ...
    stgs.supplier_name AS supplier_name,
    stgs.supplier_category_key AS supplier_category_key,
    stgs.supplier_category_name AS supplier_category_name,
...
LEFT JOIN {{ref('stg_supplier')}} stgs
    ON dp.supplier_key = stgs.supplier_key
LEFT JOIN {{ref('stg_package_type')}} stgpt
    ON dp.unit_package_key = stgpt.package_type_key
LEFT JOIN {{ref('stg_package_type')}} stgopt
    ON dp.outer_package_key = stgopt.package_type_key
LEFT JOIN {{ref('stg_color')}} stgcol
    ON dp.color_key = stgcol.color_key
